...
const PLATFORMS = [
  {
    ...
    placeholder: 'Enter Instagram username or link (e.g. a2zdownloader)...',
    helper: 'Search any profile like @a2zdownloader or paste any Reel/Post link to view & download.',
  },
  {
    id: 'tiktok',
    name: 'TikTok',
    ...
    placeholder: 'Enter TikTok username or video link (e.g. @mrbeast)...',
    helper: 'Extract unwatermarked HD avatar, total likes, and public videos.',
  },
  {
    id: 'snapchat',
    name: 'Snapchat',
    ...
    placeholder: 'Enter Snapchat username (e.g. djkhaled305 or snapchat.com/add/...)...',
    helper: 'Download 1080p Bitmoji avatars, Snapcodes & public stories.',
  },
  {
    id: 'facebook',
    name: 'Facebook',
    ...
    activeBg: 'bg-blue-600 text-white shadow-blue-500/20',
    placeholder: 'Enter Facebook username or profile URL (e.g. zuck)...',
    helper: 'Inspect public Facebook page/profile avatars and follower stats.',
  },
]
...
export default function AnonymousViewerPage() {
  ...
  const handlePaste = async () => {
    try {
      ...
      if (text) {
        setQuery(text.trim())
        if (text.includes('tiktok.com')) setActivePlatform('tiktok')
        else if (text.includes('snapchat.com')) setActivePlatform('snapchat')
        else if (text.includes('facebook.com') || text.includes('fb.com')) setActivePlatform('facebook')
        else if (text.includes('instagram.com')) setActivePlatform('instagram')
      }
    } catch {}
  }
  ...
  const handleSearch = async (e?: React.FormEvent, customQuery?: string) => {
    ...
    try {
      const res = await fetch('/api/profile', {
        ...
        body: JSON.stringify({ platform: activePlatform, query: targetQuery.trim() }),
      })
            ...
            <div className="relative flex-1">
              ...
              <div className="absolute inset-y-0 right-0 pr-2 flex items-center gap-1">
                ...
                <button
                  type="button"
                  onClick={handlePaste}
                  className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 flex items-center gap-1 transition"
                >
